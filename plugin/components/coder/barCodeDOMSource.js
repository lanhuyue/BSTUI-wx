...
(function () {
  ...
  var defaultSettings = {
    ...
    barcode = {
      ...
      digitToCssRenderer: function (settings, digit, hri, mw, mh) {
        var 
          x, y, len, current,
          lines = digit.length,
        ...
        if(settings.width){
          mw = Math.floor(settings.width / columns)
          mw = mw<=0?1:mw
        }
        var 
          // _s = Number(settings.width / columns /mw).toFixed(1);
          _margin_left = (settings.width- columns *mw)/2;
        // var content = '<div style="width:auto;transform-origin:0 0;transform:scaleX(' + _s+');">',
        var content = '<div style="margin-left:' + _margin_left+'px">',
      ...
      digitToCss: function (settings, digit, hri) {
        var w = intval(settings.barWidth),
          h = intval(settings.barHeight);
        return this.digitToCssRenderer(settings, this.bitStringTo2DArray(digit), hri, w, h);
      },
      // css 2D barcode renderer
      digitToCss2D: function (settings, digit, hri) {
        var s = intval(settings.moduleSize);
        return this.digitToCssRenderer(settings, digit, hri, s, s);
      },

    };

  Barcode = function (data, type, settings) {
    ...
    data = Object.assign({ crc: true, rect: false }, typeof data === "object" ? data : { code: data });
    ...
    if (code) {
      ...
      if (digit.length) {
        ...
        return barcode[fname](settings, digit, hri);
      }
    }
    return this;
  };
